...
public class LazyParseByteCacheTest {
    ...
    public void setUp() throws Exception {
        ...
        ApplebyteSerializer bs = new ApplebyteSerializer(unitTestParams);
        
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bs.serialize(tx1, bos);
        tx1BytesWithHeader = bos.toByteArray();
        tx1Bytes = tx1.applebyteSerialize();
        
        bos.reset();
        bs.serialize(tx2, bos);
        tx2BytesWithHeader = bos.toByteArray();
        tx2Bytes = tx2.applebyteSerialize();
        
        bos.reset();
        bs.serialize(b1, bos);
        b1BytesWithHeader = bos.toByteArray();
        b1Bytes = b1.applebyteSerialize();
    }
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
    	...
    	//message structure.
    	ApplebyteSerializer bsRef = new ApplebyteSerializer(unitTestParams, false, false);
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
    	
    	ApplebyteSerializer bs = new ApplebyteSerializer(unitTestParams, lazy, retain);
    	Block b1;
    	Block bRef;
    	...
    	//verify our reference ApplebyteSerializer produces matching byte array.
    	bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
    	...
    	//reference serializer to produce comparison serialization output after changes to
    	//message structure.
    	ApplebyteSerializer bsRef = new ApplebyteSerializer(params, false, false);
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
    	
    	ApplebyteSerializer bs = new ApplebyteSerializer(params, lazy, retain);
    	Transaction t1;
    	Transaction tRef;
    ...
    private void serDeser(ApplebyteSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
