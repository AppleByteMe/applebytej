...
/**
 * <p>Provides a standard implementation of a Applebyte URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code applebyte:<address>}</li>
 * <li>{@code applebyte:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.applebyte.it/wiki/BIP_0021">BIP 0021</a>
 */
public class ApplebyteURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(ApplebyteURI.class);
    ...
    public static final String APPLEBYTE_SCHEME = "applebyte";
    ...
    /**
     * Constructs a new ApplebyteURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws ApplebyteURIParseException if the URI is not syntactically or semantically valid.
     */
    public ApplebyteURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws ApplebyteURIParseException If the input fails Applebyte URI syntax and semantic checks.
     */
    public ApplebyteURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new ApplebyteURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  applebyte:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form applebyte://address?....
        // We support both until Ben fixes his code.
        
        // Remove the applebyte scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : applebyte:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("applebyte://")) {
            schemeSpecificPart = input.substring("applebyte://".length());
        } else if (input.startsWith("applebyte:")) {
            schemeSpecificPart = input.substring("applebyte:".length());
        } else {
            throw new ApplebyteURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new ApplebyteURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new ApplebyteURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a Applebyte address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new ApplebyteURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new ApplebyteURIParseException("Malformed Applebyte URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new ApplebyteURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The Applebyte Address from the URI
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("ApplebyteURI[");
        boolean first = true;
    ...
    public static String convertToApplebyteURI(Address address, BigInteger amount, String label, String message) {
        return convertToApplebyteURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Applebyte URI builder using known good fields.
     * 
     * @param address The Applebyte address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Applebyte URI
     */
    public static String convertToApplebyteURI(String address, BigInteger amount, String label, String message) {
        ...
        builder.append(APPLEBYTE_SCHEME).append(":").append(address);
        
        boolean questionMarkHasBeenOutput = false;
        
        if (amount != null) {
            builder.append(QUESTION_MARK_SEPARATOR).append(FIELD_AMOUNT).append("=");
            builder.append(Utils.applebyteValueToPlainString(amount));
