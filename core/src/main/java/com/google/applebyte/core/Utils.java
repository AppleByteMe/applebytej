...
/**
 * A collection of various utility methods that are helpful for working with the Applebyte protocol.
 * To enable debug logging from the library, run with -Dapplebytej.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using Applebyte keys. */
    public static final String APPLEBYTE_SIGNED_MESSAGE_HEADER = "Applebyte Signed Message:\n";

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a Applebyte.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Applebyte.
    ...
    /**
     * How many "nanocoins" there are in 0.01 Applebytes.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Applebyte.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
     ...
     * standard procedure in Applebyte. The resulting hash is in big endian form.
     */
     ...
     */
    public static String applebyteValueToFriendlyString(BigInteger value) {
     ...
     */
    public static String applebyteValueToPlainString(BigInteger value) {
        if (value == null) {
     ...
     *
     * <tt><p>[24] "Applebyte Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        VarInt size = new VarInt(message.length());
        int totalSize = 1 + APPLEBYTE_SIGNED_MESSAGE_HEADER.length() + size.getSizeInBytes() + message.length();
        byte[] result = new byte[totalSize];
        int cursor = 0;
        result[cursor++] = (byte) APPLEBYTE_SIGNED_MESSAGE_HEADER.length();
        byte[] bytes = APPLEBYTE_SIGNED_MESSAGE_HEADER.getBytes(Charset.forName("UTF-8"));
