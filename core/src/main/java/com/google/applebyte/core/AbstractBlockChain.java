 ...
 * the lightweight SPV mode, a {@link com.google.applebyte.store.BoundedOverheadBlockStore} may be a good choice.</p>
 *
 ...
 * {@link com.google.applebyte.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Applebyte
    ...
    private boolean add(Block block, Set<Sha256Hash> filteredTxHashList, List<Transaction> filteredTxn, boolean tryConnecting)
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/applebytej/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    if (clone)
                        tx = new Transaction(tx.params, tx.applebyteSerialize());
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in applebyte-qt that means mindiff blocks are accepted when time
        // goes backwards.
