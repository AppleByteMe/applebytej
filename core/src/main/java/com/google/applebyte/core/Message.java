...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Applebyte proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param msg Applebyte protocol formatted byte array containing message content.
     * @param offset The location of the first msg byte within the array.
     * @param protocolVersion Applebyte protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = applebyteSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Applebyte protocol.
    ...
    /**
     * Should only used by ApplebyteSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by ApplebyteSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeApplebyteSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] applebyteSerialize() {
        byte[] bytes = unsafeApplebyteSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the applebyte wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeApplebyteSerialize() {
        ...
        try {
            applebyteSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to applebyteSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the applebyte wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void applebyteSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use applebyteSerialize().
     */
    void applebyteSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented applebyteSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so ApplebyteSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
