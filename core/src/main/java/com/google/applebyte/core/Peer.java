...
/**
 * A Peer handles the high level communication with a Applebyte node.
 *
 * <p>{@link Peer#getHandler()} is part of a Netty Pipeline with a Applebyte serializer downstream of it.
 */
public class Peer {
    ...
    private static class GetDataRequest {
        ...
        // a nasty hack that relies on the fact that applebyte-qt is single threaded and processes messages in order.
    ...
    class PeerHandler extends SimpleChannelHandler {
        ...
        /** Handle incoming Applebyte messages */
        @Override
    ...
    /** Returns the Netty Pipeline stage handling the high level Applebyte protocol. */
     ...
     * Once the pong is received the value returned by {@link com.google.applebyte.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link com.google.applebyte.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link com.google.applebyte.core.Peer#ping()} has never
