...
import com.google.applebyte.crypto.KeyCrypterScrypt;
import org.applebytej.wallet.Protos.Wallet.EncryptionType;
import org.spongycastle.crypto.params.KeyParameter;

import com.google.applebyte.core.TransactionConfidence.ConfidenceType;
import com.google.applebyte.core.WalletTransaction.Pool;
import com.google.applebyte.crypto.KeyCrypter;
import com.google.applebyte.crypto.KeyCrypterException;
import com.google.applebyte.store.WalletProtobufSerializer;
import com.google.applebyte.utils.Locks;
...
import static com.google.applebyte.core.Utils.applebyteValueToFriendlyString;
 ...
 * of the Applebyte protocol.</p>
 *
 * <p>To learn more about this class, read <b><a href="http://code.google.com/p/applebytej/wiki/WorkingWithTheWallet">
 ...
 * other objects, see the <a href="http://code.google.com/p/applebytej/wiki/GettingStarted">Getting started</a> tutorial
 * on the website to learn more about how to set everything up.</p>
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of applebytej,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.applebyte.core.Wallet.AutosaveEventListener)}
...
public class Wallet implements Serializable, BlockChainListener {
     ...
     * that handles this in Applebyte C++ is complicated. Satoshis code will not allow you to spend unconfirmed coins,
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s BTC from our own wallet," +
                        " and sends us %s BTC", tx.getHashAsString(), Utils.applebyteValueToFriendlyString(valueSentFromMe),
                        Utils.applebyteValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType, boolean reorg) throws VerificationException {
        ...
        if (!reorg) {
            log.info("Received tx {} for {} BTC: {}", new Object[]{sideChain ? "on a side chain" : "",
                    applebyteValueToFriendlyString(valueDifference), tx.getHashAsString()});
        }
        ...
        log.info("Balance is now: " + applebyteValueToFriendlyString(getBalance()));
    ...
    public static class SendResult {
        /** The Applebyte transaction message that moves the money. */
    ...
    public static class SendRequest {
         ...
         * is how mining is incentivized in later years of the Applebyte system when inflation drops. It also provides
         * a way for people to prioritize their transactions over others and is used as a way to make denial of service
         * attacks expensive. Some transactions require a fee due to their structure - currently applebytej does not
     ...
     * followed by {@link Wallet#completeTx(com.google.applebyte.core.Wallet.SendRequest)} and returning the requests
     ...
     *
     * @param address       The Applebyte address to send the money to.
     ...
     * {@link Wallet#completeTx(com.google.applebyte.core.Wallet.SendRequest)} to make it valid.
     *
    ...
    public boolean completeTx(SendRequest req) {
        ...
        try {
            ...
            log.info("Completing send tx with {} outputs totalling {}",
                    req.tx.getOutputs().size(), applebyteValueToFriendlyString(value));
            ...
            if (selection.valueGathered.compareTo(value) < 0) {
                log.warn("Insufficient value in wallet for send, missing " +
                        applebyteValueToFriendlyString(value.subtract(selection.valueGathered)));
            ...
            if (change.compareTo(BigInteger.ZERO) > 0) {
                ...
                log.info("  with {} coins change", applebyteValueToFriendlyString(change));
            ...
            // Check size.
            int size = req.tx.applebyteSerialize().length;
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.applebyte.core.Wallet.AutosaveEventListener)}
    ...
    public String toString(boolean includePrivateKeys, AbstractBlockChain chain) {
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s BTC in:%n", applebyteValueToFriendlyString(getBalance())));
    ...
    private void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
        ...
        for (Transaction tx : transactionMap.values()) {
            try {
                ...
                builder.append(Utils.applebyteValueToFriendlyString(tx.getValueSentFromMe(this)));
                builder.append(" and receives ");
                builder.append(Utils.applebyteValueToFriendlyString(tx.getValueSentToMe(this)));
                builder.append(", total value ");
                builder.append(Utils.applebyteValueToFriendlyString(tx.getValue(this)));
                builder.append(".\n");
            } catch (ScriptException e) {
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.applebyteValueToFriendlyString(getBalance()));
     ...
     * {@link com.google.applebyte.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.google.applebyte.crypto.KeyCrypter,
     ...
     * {@link com.google.applebyte.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all applebyte controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.google.applebyte.crypto.KeyCrypter,
            ...
            for (int i = 0; i < tx.getOutputs().size(); i++) {
                ...
                try {
                    if (out.isMine(this) && out.getScriptPubKey().isSentToRawPubKey()) {
                        ...
                        filter.insert(outPoint.applebyteSerialize());
                    }
                } catch (ScriptException e) {
                    throw new RuntimeException(e); // If it is ours, we parsed the script corectly, so this shouldn't happen
                }
