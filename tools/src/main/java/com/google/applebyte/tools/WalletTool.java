...
package com.google.applebyte.tools;
...
import com.google.applebyte.store.*;
...
/**
 * A command line tool for manipulating wallets and working with Applebyte.<p>
 */
public class WalletTool {
    ...
    public static class Condition {
        ...
        public boolean matchApplebytes(BigInteger comparison) {
            try {
    ...
    public static void main(String[] args) throws Exception {
        ...
        switch (netFlag.value(options)) {
            ...
            case TEST: 
                ...
                discovery = new IrcDiscovery("#applebyteTEST3");
                break;
            default:
                throw new RuntimeException("Unreachable.");
        }
    ...
    private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) {
        try {
            ...
            if (!wallet.completeTx(req)) {
                System.err.println("Insufficient funds: have " + Utils.applebyteValueToFriendlyString(wallet.getBalance()));
                return;
            }
            try {
                if (lockTimeStr != null) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchApplebytes(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchApplebytes(balance)) {
                            System.out.println(Utils.applebyteValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Applebyte address of the right network parameters.");
                return;
            }            
        }
        if (key == null) {
